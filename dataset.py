...
class Cityscapes(Dataset):
    def __init__(self, root, split='train', crop_size=None, mean=city_mean, std=city_std, ignore_label=255):
        ...
        search_images = os.path.join(root, 'leftImg8bit', split, '*.png')

        search_labels = os.path.join(root, 'gtFine', split, '*.png')
        search_grads = os.path.join(root, 'boundary', split, '*.png')
        search_boundaries = os.path.join(root, 'boundary', split, '*.png')
        
        print('search_images',search_images)
        print('search_labels',search_labels)
        print('search_grads',search_grads)
        print('search_boundaries',search_boundaries)
...
def creat_dataset(root, num_classes=3, ignore_label=255):
    search_path = os.path.join(root, 'leftImg8bit','*','*.png')
    ...
    search_path = os.path.join(root, 'gtFine','*', '*.png')
    if not glob.glob(search_path):
    ...
    if not glob.glob(search_path):
        ...
        pool.close()
        pool.join()
    search_path = os.path.join(root, 'boundary','*', '*.png')
    print('search_path',search_path)
    if not glob.glob(search_path):
